...
#remove outlier samples from annotated sites and only keep shared pangenome contigs
...
grep -E 'synonymous_variant|#' vcf_files/annotated_output_biallelic_goodcontigs_remoutliers.vcf > vcf_files/annotated_output_biallelic_goodcontigs_remoutliers_synonymous.vcf

#############
bcftools filter -i 'AC > 9' vcf_files/annotated_output_biallelic_goodcontigs_remoutliers_synonymous.vcf | sed 's/\.\:\./0\:255\,0/g'  > vcf_files/annotated_output_biallelic_goodcontigs_remoutliers_synonymous_MAF10_refedit.vcf

cat <(grep -e '#' vcf_files/annotated_output_biallelic_goodcontigs_remoutliers_synonymous_MAF10_refedit.vcf) <(grep -Ev '#' vcf_files/annotated_output_biallelic_goodcontigs_remoutliers_synonymous_MAF10_refedit.vcf | sed -n '0~10p') > vcf_files/annotated_output_biallelic_goodcontigs_remoutliers_synonymous_MAF10_refedit_subsample.vcf
...
#for genopred
bcftools filter -i 'AC > 7' vcf_files/annotated_output_biallelic_goodcontigs_remoutliers.vcf  | \
bcftools +prune -m 0.25 -w 50000 | \
sed 's/\.\:\./0\:255\,0/g' > vcf_files/annotated_output_biallelic_MAF7_refedit_goodcontigs_ldpruned.vcf


bcftools query -H -f "%CHROM\t%POS\t%ALT\t%AC\t%DP[\t%GT]\n"  \
vcf_files/annotated_output_biallelic_MAF7_refedit_goodcontigs_ldpruned.vcf \
-o vcf_files/annotated_output_biallelic_MAF7_refedit_goodcontigs_ldpruned_remoutliers.txt
